...
namespace djv
{
    namespace ViewApp
    {
        ...
        void AudioSystem::_init(const std::shared_ptr<System::Context>& context)
        {
            ...
            _addShortcut(DJV_TEXT("shortcut_audio_decrease_volume"), GLFW_KEY_8, UI::getSystemModifier());
            _addShortcut(DJV_TEXT("shortcut_audio_increase_volume"), GLFW_KEY_7, UI::getSystemModifier());
            _addShortcut(DJV_TEXT("shortcut_audio_mute"), GLFW_KEY_9, UI::getSystemModifier());
            ...
            if (auto fileSystem = context->getSystemT<FileSystem>())
            {
                    ...
                    [weak](const std::shared_ptr<Media> & value)
                {
                    if (auto system = weak.lock())
                    {
                        ...
                        if (value)
                        {
                            system->_p->infoObserver = Observer::Value<AV::IO::Info>::create(
                                value->observeInfo(),
                            ...
                            system->_p->volumeObserver = Observer::Value<float>::create(
                                value->observeVolume(),
                                [weak](float value)
                            {
                                if (auto system = weak.lock())
                            ...
                            system->_p->muteObserver = Observer::Value<bool>::create(
                                value->observeMute(),
                                [weak](bool value)
                            {
                                if (auto system = weak.lock())
