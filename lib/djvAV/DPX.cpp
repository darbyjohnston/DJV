...
namespace djv
{
    namespace AV
    {
        namespace DPX
        {
                ...
                const std::shared_ptr<System::TextSystem>& textSystem)
            {
                ...
                {
                    info.videoSpeed = fromSpeed(out.tv.frameRate);
                    std::stringstream ss;
                    ss << out.tv.frameRate;
    ...
    rapidjson::Value toJSON(const AV::DPX::Options& value, rapidjson::Document::AllocatorType& allocator)
    {
    ...
    void fromJSON(const rapidjson::Value& value, AV::DPX::Options& out)
    {
        if (value.IsObject())
        {
