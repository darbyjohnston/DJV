...
namespace djv
{
    namespace AV
    {
        namespace Time
        {
            ...
            uint32_t frameToTimecode(Math::Frame::Number frame, const Math::IntRational& r)
            {
                ...
                frame -= static_cast<Math::Frame::Number>(hour) * static_cast<double>(f) * static_cast<Math::Frame::Number>(60) * static_cast<Math::Frame::Number>(60);
                const int minute = static_cast<int>(frame / (f * 60));
                frame -= static_cast<Math::Frame::Number>(minute) * static_cast<double>(f) * static_cast<Math::Frame::Number>(60);
                const int second = static_cast<int>(frame / f);
                frame -= static_cast<Math::Frame::Number>(second) * static_cast<double>(f);
    ...
    rapidjson::Value toJSON(AV::Time::Units value, rapidjson::Document::AllocatorType& allocator)
    {
        std::stringstream ss;
        ss << value;
    ...
    void fromJSON(const rapidjson::Value& value, AV::Time::Units& out)
    {
        if (value.IsString())
        {
