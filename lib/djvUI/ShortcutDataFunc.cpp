...
namespace djv
{
    namespace UI
    {
        ...
        std::string modifierToString(int key)
        {
        ...
        int modifierFromString(const std::string& value)
        {
            int out = 0;
        ...
        std::string getText(const ShortcutData& shortcutData, const std::shared_ptr<System::TextSystem>& textSystem)
        {
            return getText(shortcutData.key, shortcutData.modifiers, textSystem);
        }

        std::string getText(int key, int keyModifiers, const std::shared_ptr<System::TextSystem>& textSystem)
        {
    ...
    rapidjson::Value toJSON(const UI::ShortcutData& value, rapidjson::Document::AllocatorType& allocator)
    {
        ...
        out.AddMember("Key", toJSON(UI::keyToString(value.key), allocator), allocator);
        std::vector<std::string> modifiers;
        if (value.modifiers & GLFW_MOD_SHIFT)
        {
            modifiers.push_back(UI::modifierToString(GLFW_MOD_SHIFT));
        }
        if (value.modifiers & GLFW_MOD_CONTROL)
        {
            modifiers.push_back(UI::modifierToString(GLFW_MOD_CONTROL));
        }
        if (value.modifiers & GLFW_MOD_ALT)
        {
            modifiers.push_back(UI::modifierToString(GLFW_MOD_ALT));
        }
        if (value.modifiers & GLFW_MOD_SUPER)
        {
            modifiers.push_back(UI::modifierToString(GLFW_MOD_SUPER));
        }
        ...
        if (value.IsObject())
        {
            for (const auto& i : value.GetObject())
            {
                ...
                else if (0 == strcmp("Modifiers", i.name.GetString()))
                {
                    ...
                    {
                        out.modifiers |= UI::modifierFromString(j);
                    }
                }
            }
        }
