...
namespace djv
{
    namespace UI
    {
        struct MenuBar::Private
        {
            ...
            std::map<std::shared_ptr<Menu>, std::shared_ptr<Observer::Value<std::string> > > iconObservers;
            std::map<std::shared_ptr<Menu>, std::shared_ptr<Observer::Value<std::string> > > textObservers;
        ...
        void MenuBar::_init(const std::shared_ptr<System::Context>& context)
        {
            ...
            setBackgroundColorRole(ColorRole::BackgroundHeader);
        ...
        void MenuBar::addChild(const std::shared_ptr<IObject>& value)
        {
            ...
            if (auto context = getContext().lock())
            {
                if (auto menu = std::dynamic_pointer_cast<Menu>(value))
                {
                    ...
                    p.iconObservers[menu] = Observer::Value<std::string>::create(
                        menu->observeIcon(),
                    ...
                    p.textObservers[menu] = Observer::Value<std::string>::create(
                        menu->observeText(),
                        [button](const std::string& value)
                        {
