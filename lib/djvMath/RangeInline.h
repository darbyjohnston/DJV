...
namespace djv
{
    ...
    template<typename T>
    inline std::ostream& operator << (std::ostream& os, const Math::Range<T>& value)
    {
        os << value.getMin() << " ";
        os << value.getMax();
        return os;
    ...
    template<typename T>
    inline std::istream& operator >> (std::istream& is, Math::Range<T>& out)
    {
        try
        {
